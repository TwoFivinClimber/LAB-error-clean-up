  ...
  divId === '#voldy'
    ...
    : student.crest}' 
      ...
      <div class='card-img-overlay'>
        ...
        ${divId} === '#voldy' ? '<p class='card-text'>Death Eater</p>' : '<p class='card-text'>${
  student.house
}</p>'; <button type='button' id='expel--${
  student.id
}' class='btn btn-danger btn-sm'>Expel</button>`;
  });
...
const filterBtnRow = () => {
  let domString = '';
  domString += `<div class='btn-group' role='group' aria-label='Basic example'>
...
export {
  htmlStructure, header, studentsOnDom, startSortingBtn, studentAreas, filterBtnRow
};
